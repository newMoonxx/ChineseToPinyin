...
namespace ChineseToPinyin
{
    public class PinyinHelper
    {
		...
		3. 来源于各种网络数据的整合
		*/
        private static string[,] lastNameTable = new string[g_length_lastNameTable, 3]
        {
            { "赵" , "zhao" , "z" },
            { "钱" , "qian" , "q" },
            { "孙" , "sun" , "s" },
		...
		3. 多音字选中读音规则：
		   ...
		   e. 需要上下文联系的，这种字没法了，只能查表，这里的输出原则遵循（abcd）
				...
				{ "卜" , "bu" },        // bo ，   占卜、萝卜
		*/
        private static string[,] multiToneWordTable = new string[g_length_multiToneWordTable, 3]
        {
            { "厑" , "e" , "e" },         // si a
            { "吖" , "ya" , "y" },        // a
            { "阿" , "a" , "a" },         // e
		...
		*/
        ...
        /// 获取完整姓名的拼音
        /// </summary>
        /// <param name="inFullName">姓名</param>
        /// <param name="fullPinyin">拼音/首字母标记</param>
        /// <returns></returns>
        public static string GetFullNamePinyin(string inFullName, bool fullPinyin = true)
        {
            FullNameSplit(inFullName, out string lastName, out string name);
            return GetNamePinyin(lastName, true, fullPinyin) + GetNamePinyin(name, false, fullPinyin);
        }
        ...
        /// </example>
        /// 
        public static string GetNamePinyin(string inStr, bool isLastName, bool fullPinyin = true)
        {
            string outStr = "";
            ...
            if (isLastName)
            {
                ...
                for (int i = 0; i < g_length_lastNameTable; i++)
                {
                    ...
                    {
                        outStr = lastNameTable[i, fullPinyin ? 1: 2];
                        break;
                    }
                }
            }
            ...
            if (outStr == "")
            {
                ...
                {
                    outStr += GetNoRepeatPinyinForWord(inStr.Substring(i, 1), fullPinyin);
                }
            }
        ...
        private static string GetNoRepeatPinyinForWord(string inWord, bool fullPinyin)
        {
            //只能输入一个字符
            if (inWord.Length > 1)
            {
            ...
            for (int i = 0; i < g_length_multiToneWordTable; i++)
            {
                ...
                {
                    return multiToneWordTable[i, fullPinyin ? 1: 2];
                }
            }
            ...
            if (IsHanziChar(wch))
            {
                for (int index_table = 0; index_table < g_length_hanziTables; index_table++)
                {
                    ...
                    {
                        return hanziTables[index_table, fullPinyin ? 1: 0];
                    }
                }
            }

            throw new ArgumentException($"不支持的字符：\"{inWord}\"");
        }

        public static void GetComPinyinForStr(string inStr, out string outFrist_py, out string outFull_py)
        {
            outFrist_py = "";
            outFull_py = "";
            ...
            for (int i_word = 0; i_word < numOfWord; i_word++)
            {
                List<string> fristPy;
                List<string> fullPy;
                GetAllPinyinForWord(inStr.Substring(i_word, 1), out fristPy, out fullPy);
        ...
        private static void GetAllPinyinForWord(string inWord, out List<string> outFrist_py, out List<string> outFull_py)
        {
